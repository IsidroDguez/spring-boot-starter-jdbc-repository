...
public interface OracleRawQueryRepository extends QueryContract {

    @Override
    @JdbcRawQuery(value = "select * from test.employee")
    List<Employee> getAllEmployeeHardcoded();

    @Override
    @JdbcRawQuery(value = "${properties.employee.query.all.oracle}")
    Set<Employee> getAllEmployeeSetFromProperties();

    @Override
    @JdbcRawQuery(value = "${yml.employee.query.all-oracle}")
    Employee[] getAllEmployeeArrayFromYml();

    @Override
    @JdbcRawQuery(value = "file:/jdbc-employee-oracle.sql")
    Stream<Employee> getAllEmployeeStreamFromFile();

    @Override
    @JdbcRawQuery(value = "select * from test.employee where id > :id")
    List<Employee> getAllEmployeeListWithCondition(Integer id);

    @Override
    @JdbcRawQuery(value = "select * from test.employee where id = :idParam")
    ...
    @Override
    @JdbcRawQuery(value = "select * from test.employee where id = :id", mapper = EmployeeRowMapper.class)
    Employee getOneEmployeeWithConditionAndRowMapper(Integer id);

    @Override
    @JdbcRawQuery(value = "select * from test.employee e " +
            "inner join test.salary s on s.employee_id = e.id where e.id = :id", mapper = EmployeeAndSalaryRowMapper.class)
    ...
    @Override
    @JdbcRawQuery(value = "select e.id, e.first_name, e.last_name, s.amount from test.employee e " +
            "inner join test.salary s on s.employee_id = e.id where e.id = :id")
    EmployeeAndSalaryProjection getOneEmployeeWithConditionWithComplexProjection(Integer id);

    @Override
    @JdbcRawQuery(value = "select e.id, e.first_name, e.last_name, t.title from test.employee e " +
            "inner join test.title t on t.employee_id = e.id where e.id = :id", mapper = EmployeeAndTitleProjectionRowMapper.class)
    EmployeeAndTitleProjection getOneEmployeeWithConditionWithComplexProjectionImpl(Integer id);

    @Override
    @JdbcRawQuery(value = "select birth_date from test.employee where id = :id")
